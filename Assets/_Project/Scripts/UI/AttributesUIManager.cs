...
public class AttributesUIManager : MonoBehaviour
{
    ...
    private string lastDisplayedClassId;
    ...
    private void RefreshPlayerPortrait()
    {
        ...
        string classId = playerClass != null ? playerClass.GetClassId() : string.Empty;
        Sprite portrait = playerClass != null ? playerClass.portrait : null;

        if (classId == lastDisplayedClassId && portrait == lastDisplayedPlayerPortrait)
            return;

        playerPortraitImage.sprite = portrait;
        playerPortraitImage.enabled = portrait != null || !hidePlayerPortraitWhenMissing;
        lastDisplayedClassId = classId;
    ...
    private void RefreshPlayerName()
    {
        ...
        PlayerClassData playerClass = ResolveSelectedClass();
        string resolvedName = ResolveSelectedClassName(playerClass);
    ...
    private string ResolveSelectedClassName(PlayerClassData playerClass)
    {
        ...
        return string.IsNullOrWhiteSpace(selectedClassId) ? "Player" : selectedClassId.Trim();
    }

    private PlayerClassData ResolveClassById(string classId)
    {
        if (playerClassDatabase == null || string.IsNullOrWhiteSpace(classId))
            return null;

        var classes = playerClassDatabase.Classes;
        if (classes == null)
            return null;
        ...
        {
            var candidate = classes[i];
            if (candidate == null)
                continue;

            if (string.Equals(candidate.GetClassId(), normalizedId, System.StringComparison.OrdinalIgnoreCase))
                return candidate;
        }
    ...
    private PlayerClassData ResolveSelectedClass()
    {
        if (playerClassDatabase == null)
            return null;

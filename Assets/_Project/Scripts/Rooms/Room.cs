...
public class Room : MonoBehaviour
{
    ...
    private SavedDungeonRoomState state; private RoomRuleContext context; private GameObject spawnedPortal; private bool initialized;
    ...
    private void InitializeRuntime()
    {
        ...
        state=DungeonRunStateController.Active!=null?DungeonRunStateController.Active.GetRoom(RuntimeId):new SavedDungeonRoomState{roomId=RuntimeId,rules=Array.Empty<SavedDungeonRuleState>()};context=new RoomRuleContext(this,state);if(isLocked)doorLocks.Add("legacy-entry");
    ...
    {if(enemy==null)return;if(!activeEnemies.Contains(enemy))activeEnemies.Add(enemy);if(!encounterEnemies.TryGetValue(ownerId??"legacy",out var list)){list=new List<GameObject>();encounterEnemies[ownerId??"legacy"]=list;}if(!list.Contains(enemy))list.Add(enemy);enemy.SetActive(false);}
    public void AdoptEncounter(string oldOwner,string newOwner){if(encounterEnemies.TryGetValue(oldOwner,out var list)){encounterEnemies.Remove(oldOwner);encounterEnemies[newOwner]=list;}}
    ...
    private void EvaluateCompletion(){if(roomCleared)return;foreach(var rule in rules)if(rule!=null&&rule.BlocksRoomCompletion&&!rule.IsResolved)return;roomCleared=true;state.completed=true;state.encounterInProgress=false;doorLocks.Clear();RefreshDoors();foreach(var rule in rules)if(rule!=null)rule.OnRoomCompleted();QuestEvents.Raise(QuestObjectiveEventType.ClearRoom,ResolveQuestTargetId(),ResolveQuestTargetTag());TrySpawnFloorPortal();}

...
public sealed class ShrineEncounter : MonoBehaviour
{
    ...
    {
        if(room==null||choice==null||selected.Count>=maxSelections||selected.Contains(choice.ChoiceId))return false;PlayerStats stats=player!=null?player.GetComponentInParent<PlayerStats>():PlayerStats.instance;
        if(stats==null||!choice.AreRequirementsMet(stats)||!DungeonCostTransaction.CanPay(choice.Costs,stats)||!choice.CanApplyOutcomes(stats,room))return false;
        if(!DungeonCostTransaction.TryPay(choice.Costs,stats))return false;
        choice.ApplyOutcomes(stats,room);selected.Add(choice.ChoiceId);state.payload=string.Join(",",selected);state.completed=selected.Count>=maxSelections;room.SaveExternalState(state);Refresh();return true;
    }
...
public sealed class ShrineChoice:MonoBehaviour,IInteractable
{
    ...
    internal bool CanApplyOutcomes(PlayerStats stats,Room room)
    {
        if(outcomes==null)return true;for(int i=0;i<outcomes.Length;i++)if(outcomes[i]!=null&&!outcomes[i].CanApply(stats,DungeonDeterminism.Create(DungeonRunStateController.Active?.RunSeed??string.Empty,room.Floor,room.RuntimeId,"shrine:"+choiceId+":"+i)))return false;return true;
    }
    internal void ApplyOutcomes(PlayerStats stats,Room room)
    {if(outcomes==null)return;for(int i=0;i<outcomes.Length;i++)if(outcomes[i]!=null)outcomes[i].Apply(stats,DungeonDeterminism.Create(DungeonRunStateController.Active?.RunSeed??string.Empty,room.Floor,room.RuntimeId,"shrine:"+choiceId+":"+i));}
...
public sealed class RiskRewardInteraction:MonoBehaviour,IInteractable
{
    ...
    public void Interact(GameObject player)
    {
        ...
        if(!DungeonCostTransaction.CanPay(costs,stats)||!CanApplyOutcomes(stats))return;if(!DungeonCostTransaction.TryPay(costs,stats))return;
        if(outcomes!=null)for(int i=0;i<outcomes.Length;i++)if(outcomes[i]!=null)outcomes[i].Apply(stats,DungeonDeterminism.Create(DungeonRunStateController.Active?.RunSeed??string.Empty,room.Floor,room.RuntimeId,interactionId+":"+i));state.completed=true;room.SaveExternalState(state);
    }
    private bool CanApplyOutcomes(PlayerStats stats)
    {if(outcomes==null)return true;for(int i=0;i<outcomes.Length;i++)if(outcomes[i]!=null&&!outcomes[i].CanApply(stats,DungeonDeterminism.Create(DungeonRunStateController.Active?.RunSeed??string.Empty,room.Floor,room.RuntimeId,interactionId+":"+i)))return false;return true;}

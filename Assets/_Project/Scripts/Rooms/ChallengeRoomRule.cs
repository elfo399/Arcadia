...
public sealed class ChallengeRoomRule : RoomRule, IInteractable
{
    ...
    public void Interact(GameObject player) { if(!active&&!IsCompleted&&!IsFailed) StartChallenge(); }
    public string GetPrompt() => active||IsCompleted||IsFailed ? string.Empty : prompt;
    private void StartChallenge() { if(waves.Count==0){Debug.LogError($"[ChallengeRoomRule] {name} requires at least one wave.",this);return;} active=true; remaining=timeLimitSeconds; Context.Room.BeginCombat(this); StartNextWave(); }
    private void StartNextWave()
    {
        ...
        var random=Context.CreateRandom(RuleId+":challenge:"+waveIndex); foreach(var point in points) point.Spawn(wave.enemyPools[random.Next(wave.enemyPools.Count)],random); Context.Room.WakeUpEnemies();
    }
    private void Update() { if(active && mode==DungeonChallengeMode.TimedKill && (remaining-=Time.deltaTime)<=0f) EndFailure(); }
    public override void OnEnemyDied(GameObject enemy) { if(active && Context.Room.ActiveEnemyCount==0) StartNextWave(); }
    private void HandlePlayerDamage(float amount) { if(active && mode==DungeonChallengeMode.PerfectCombat && amount>0f) EndFailure(); }
    private void EndFailure() { if(!active)return; active=false; Fail(); if(failureCompletesRoom) Complete(); }

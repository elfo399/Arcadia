...
[CreateAssetMenu(fileName = "NewPlayerClass", menuName = "Arcadia/Player Class")]
public class PlayerClassData : ScriptableObject
{
    ...
    [Header("Identity")]
    [FormerlySerializedAs("characterId")] public string classId = "warrior";
    ...
    {
        return string.IsNullOrWhiteSpace(classId) ? name : classId.Trim();
    }
    ...
    private void AddBackpackItems(PlayerInventory inventory)
    {
        ...
        for (int i = 0; i < backpackItems.Length; i++)
        {
            ...
            if (assigned == 0)
                continue;
            if (assigned > 1)
                Debug.LogWarning($"[PlayerClassData] Backpack entry in {name} has multiple item references. Priority: Weapon > Magic > Armor > Usable > Item.");
